...
// build/dev/javascript/canvas_draw/canvas_draw_ffi.mjs
function documentGetElementById(id2) {
  let el2 = document.getElementById(id2);
  return el2 === null ? new Error(`no element with id ${id2}`) : new Ok(el2);
}
function canvasGetContext2d(canvas2) {
...
function contextClearRect(ctx, x, y, width2, height2) {
...
function contextArc(ctx, x, y, radius, startAngle, endAngle, counterclockwise) {
  requestAnimationFrame(() => {
    ctx.arc(x, y, radius, startAngle, endAngle, counterclockwise);
  });
  return ctx;
}
function contextFillStyle(ctx, color) {
  requestAnimationFrame(() => {
    ctx.fillStyle = color;
  });
  return ctx;
...
function canvasMousePosition(canvasId, mouseX, mouseY) {
  let canvas2 = document.getElementById(canvasId);
  let rect = canvas2.getBoundingClientRect();
  let scale = canvasScale(canvas2.width, canvas2.height, rect.width, rect.height);
  let x = (mouseX - rect.x) * scale.scaleX;
  let y = (mouseY - rect.y) * scale.scaleY;
  return [x, y];
}

// build/dev/javascript/canvas_draw/canvas_draw.mjs
var CtxReady = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var CtxNeedsInit = class extends CustomType {
};
var CtxError = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
...
var Model = class extends CustomType {
  constructor(ctx, canvas_width, canvas_height, canvas_id, mouse_x, mouse_y, pen_r, pen_color, is_drawing) {
    super();
    this.ctx = ctx;
...
var GetCanvasContext = class extends CustomType {
};
var SetCanvasContext = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var UserMouseDownInCanvas = class extends CustomType {
  constructor(x, y) {
    super();
    this.x = x;
    this.y = y;
...
var UserMouseMoveInCanvas = class extends CustomType {
  constructor(x, y) {
    super();
    this.x = x;
    this.y = y;
...
var UserMouseUpInCanvas = class extends CustomType {
  constructor(x, y) {
    super();
    this.x = x;
    this.y = y;
...
var UserChangedPenColor = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var UserChangedPenRadius = class extends CustomType {
  constructor(x0) {
    super();
    this[0] = x0;
  }
};
var UserClickedClearCanvas = class extends CustomType {
};
...
function view(model) {
  let title = h1(toList([]), toList([text2("\u2728 Draw something... \u2728")]));
  let canvas2 = canvas(
    toList([
      id("canvas"),
        ...
        (event2) => {
          return map2(
            ...
            (xy) => {
              let x = xy[0];
              let y = xy[1];
              return new UserMouseDownInCanvas(x, y);
            }
          );
        }
      ),
        ...
        (event2) => {
          return map2(
            ...
            (xy) => {
              let x = xy[0];
              let y = xy[1];
              return new UserMouseMoveInCanvas(x, y);
            }
          );
        }
      ),
        ...
        (event2) => {
          return map2(
            ...
            (xy) => {
              let x = xy[0];
              let y = xy[1];
              return new UserMouseUpInCanvas(x, y);
            }
          );
        }
      )
  ...
  let pen_options = div(
    ...
    toList([
      div(
        ...
        toList([
          label(
            toList([for$("pen_radius_input")]),
            toList([text2("Pen size")])
          ),
          input(
            toList([
              on_input(
                (s) => {
                  ...
                  let n = $[0];
                  return new UserChangedPenRadius(n);
                }
              ),
              id("pen_radius_input"),
              name("pen_radius_input"),
              type_("range"),
              min("2"),
              max("24"),
              value(to_string2(model.pen_r)),
              step("2")
            ])
          )
        ])
      ...
      div(
        ...
        toList([
          label(
            toList([for$("pen_color_input")]),
            toList([text2("Pen color")])
          ),
          input(
            toList([
              on_input((s) => {
                return new UserChangedPenColor(s);
              }),
              id("pen_color_input"),
              name("pen_color_input"),
              type_("color"),
              value(model.pen_color)
            ])
          )
        ])
      )
  ...
  let clear_canvas_button = div(
    toList([]),
    toList([
      button(
        toList([on_click(new UserClickedClearCanvas())]),
        toList([text2("Clear")])
      )
    ])
  );
  return div(
    toList([]),
    toList([title, canvas2, pen_options, clear_canvas_button])
  );
}
function dispatch_draw() {
  return from2((dispatch) => {
    return dispatch(new Draw());
  });
}
function pi() {
  return 3.14159265359;
}
function get_canvas_context(id2) {
  return from2(
    (dispatch) => {
      let ctx = try$(
        documentGetElementById(id2),
        (canvas2) => {
          return map2(
            canvasGetContext2d(canvas2),
            (ctx2) => {
              return ctx2;
            }
          );
      ...
      let _pipe = (() => {
        if (ctx.isOk()) {
          let ctx$1 = ctx[0];
          return new CtxReady(ctx$1);
        } else {
          let msg = ctx[0];
          return new CtxError(msg);
        }
      })();
      let _pipe$1 = new SetCanvasContext(_pipe);
...
function clear_canvas(ctx, canvas_width, canvas_height) {
  if (ctx instanceof CtxReady) {
    let ctx$1 = ctx[0];
    return from2(
      (dispatch) => {
        let $ = contextClearRect(ctx$1, 0, 0, canvas_width, canvas_height);
  ...
  return from2(
    (dispatch) => {
      if (ctx instanceof CtxReady) {
        ...
        ignore(f(ctx$1));
        return dispatch(new DoNothing());
      } else if (ctx instanceof CtxNeedsInit) {
        dispatch(new GetCanvasContext());
        return dispatch(new Draw());
      } else {
        let error = ctx[0];
        windowAlert(error);
...
function fill_circle(ctx, x, y, r, color) {
  return draw(
    ctx,
    (ctx2) => {
      contextFillStyle(ctx2, color);
      contextBeginPath(ctx2);
      contextArc(ctx2, x, y, r, 0, 2 * pi(), false);
      return contextFill(ctx2);
    }
  );
}
function canvas_mouse_position(canvas_id, mouse_x, mouse_y) {
  let xy = canvasMousePosition(canvas_id, mouse_x, mouse_y);
  return [xy[0], xy[1]];
}
function update2(model, msg) {
  if (msg instanceof DoNothing) {
    return [model, none()];
  } else if (msg instanceof GetCanvasContext) {
    return [model, get_canvas_context(model.canvas_id)];
  } else if (msg instanceof SetCanvasContext) {
    let ctx = msg[0];
    return [model.withFields({ ctx }), none()];
  } else if (msg instanceof UserMouseDownInCanvas) {
    let x = msg.x;
    let y = msg.y;
    let model$1 = model.withFields({ mouse_x: x, mouse_y: y, is_drawing: true });
    return [model$1, dispatch_draw()];
  } else if (msg instanceof UserMouseMoveInCanvas) {
    let x = msg.x;
    let y = msg.y;
    let model$1 = model.withFields({ mouse_x: x, mouse_y: y });
    let eff = (() => {
      let $ = model$1.is_drawing;
      if ($) {
        return dispatch_draw();
      } else {
        return none();
      }
    })();
    return [model$1, eff];
  } else if (msg instanceof UserMouseUpInCanvas) {
    let x = msg.x;
    let y = msg.y;
    let model$1 = model.withFields({ mouse_x: x, mouse_y: y, is_drawing: false });
    return [model$1, dispatch_draw()];
  } else if (msg instanceof Draw) {
    let pen_xy = canvas_mouse_position(
      model.canvas_id,
      model.mouse_x,
      model.mouse_y
    );
    ...
    return [
      model,
      fill_circle(model.ctx, pen_x, pen_y, model.pen_r, model.pen_color)
    ];
  } else if (msg instanceof UserChangedPenColor) {
    let s = msg[0];
    return [model.withFields({ pen_color: s }), none()];
  } else if (msg instanceof UserChangedPenRadius) {
    let n = msg[0];
    return [model.withFields({ pen_r: n }), none()];
  } else {
    return [
      model,
      clear_canvas(model.ctx, model.canvas_width, model.canvas_height)
    ];
  }
}
function main() {
